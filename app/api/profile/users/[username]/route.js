  ...
  try {
    ...
    const [stats] = await query(`
      SELECT 
        ...
        COALESCE(SUM(p_likes.likes_count), 0) as total_likes_received,
        COUNT(DISTINCT post_comments.id) as total_comments_received
      FROM users u
      LEFT JOIN posts p ON u.id = p.user_id AND p.published = true
      LEFT JOIN comments c ON u.id = c.user_id
      LEFT JOIN (
        SELECT p2.id, COALESCE(SUM(cl.likes_count), 0) as likes_count
        FROM posts p2 
        LEFT JOIN comments cl ON p2.id = cl.post_id
        WHERE p2.published = true
        GROUP BY p2.id
      ) p_likes ON p.id = p_likes.id
      LEFT JOIN comments post_comments ON p.id = post_comments.post_id
      WHERE u.id = ?
      GROUP BY u.id
    `, [user.id]);
